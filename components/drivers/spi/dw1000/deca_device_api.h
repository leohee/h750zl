 ...
 *
 * @brief This call initiates the transmission without auto-FCS, input parameter indicates which TX mode is used see below
 *
 * input parameters:
 * @param mode - if mode = DWT_START_TX_IMMEDIATE - immediate TX (no response expected)
 *               if mode = DWT_START_TX_DELAYED - delayed TX (no response expected)
 ...
 *
 * input parameters:
 * @param externalmvolt - this is the a true voltage in millivolts to convert
 *
 * output parameters:
 *
